...
export const AddObjectiveQuestion = () => {
  ...
  const onSubmit = async (data: QuestionObjective) => {
    ...
    const response = await axios.post< QuestionsResponseData>(
      `http://localhost:8080/mylibrary/addQuestions`,dataRequest
    )
  ...
  return (
    <section className="relative top-20 w-[90%] mx-auto">
      <QuestionAddedInfo isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen}/>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex flex-col md:flex-row gap-4 ">
              ...
              {options.map((_, index) => (
                <div key={index} >
                  ...
                  <input
                    ...
                    required
                  />
                  <button onClick={()=>handleDeleteOption(index)} type='button' className=' m-1 w-1/12'>
                    <DeleteSVG/>
                  </button>
                  </div>
          ...
          render={({ field }) => (
            <CategoriesInput
              suggestions={['Java', 'C', 'C++', 'Python','Java script','Ruby']} 
        ...
        />
          ...
          render={({ field }) => (
            <AutocompleteInput
              suggestions={['HTML', 'Java', 'Java Script', 'Code','MS-Word','MS-EXCEL','Programming']} 
              onSelect={(selectedValues) => field.onChange(selectedValues)} 
            />
          )}
        />
