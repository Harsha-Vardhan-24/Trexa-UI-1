...
export const Mylibrary=()=>{
  ...
  const [tabledata,SetTableData]=useState<QuestionObjective[]|QuestionProgramming[]>([]);
  ...
  const columns: TableColumn<QuestionObjective |QuestionProgramming>[] = [
    {
      name: 'Questions',
      selector: (row: QuestionObjective|QuestionProgramming) => htmlToPlainText(row.questionText),
      sortable: true,
    },
    {
      name: 'Category',
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const response = await axios.post< QuestionsResponseData>(
        `http://localhost:8080/mylibrary`,filterData
      )
    ...
    if(tab==="Objective")
      {
        ...
        SetTableData(questionsData?.objective.questions?questionsData?.objective.questions:[])
        setTotalRows(questionsData?.objective.count?questionsData?.objective.count:0)
        setCategoriesCount(questionsData?.objectiveCategories?questionsData?.objectiveCategories.length:0)
        setPaginationPerPage(filterData.page.objective.recordsPerPage)
        setQuestionCategories(questionsData?.objectiveCategories?questionsData?.objectiveCategories:[])
      ...
      if(tab==="Programming")
        {
          console.log(paginationPerPage)
          SetTableData(questionsData?.programming.questions?questionsData?.programming.questions:[])
          setTotalRows(questionsData?.programming.count?questionsData?.programming.count:0)
          setCategoriesCount(questionsData?.programmingCategories?questionsData?.programmingCategories?.length:0)
          setPaginationPerPage(filterData.page.programming.recordsPerPage)
          setQuestionCategories(questionsData?.programmingCategories?questionsData?.programmingCategories:[])
        }
     
  };
    return(
        <section className="relative top-20 mx-auto w-[90%]">
          <DeleteQuestionPopUp isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen}/>
    <div className="flex flex-wrap justify-center gap-4">
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 mt-10 justify-between mb-10">
          ...
          <div className="flex gap-1">
            <button
              className={selectDificulty==="ALL"?"btn primary":"btn secondary"}
              onClick={() => handleDifficultyLevel('ALL')}
            >
              All
            </button>
            <button
              className={selectDificulty==="EASY"?"btn primary":"btn secondary"}
              onClick={() => handleDifficultyLevel('EASY')}
            >
              Easy
            </button>
            <button
              className={selectDificulty==="MEDIUM"?"btn primary":"btn secondary"}
              onClick={() => handleDifficultyLevel('MEDIUM')}
            >
              Medium
            </button>
            <button
              className={selectDificulty==="HARD"?"btn primary":"btn secondary"}
     ...
     <div>
            ...
            <div className="mb-4 border-b  border-gray-200 flex space-between">
              <ul className="admin-questions-differentiation-tabs">
                <li className="me-2">
                  <button
                    aria-current="page"
                    className={`inline-block p-4 text-primary rounded-t-lg ${activeTab==='Objective'?"font-bold bg-gray-100  active":"hover:text-green-900 hover:font-bold hover:bg-gray-50"}`}
                ...
                <li className="me-2">
                  <a
                    className={`inline-block p-4 text-primary rounded-t-lg ${activeTab==='Programming'?"font-bold bg-gray-100  active":"hover:text-green-900 hover:font-bold hover:bg-gray-50"}`}
                ...
                <li className="me-2">
                  <a
                    href="#"
                    className={`inline-block p-4 text-primary rounded-t-lg ${activeTab==='Video'?"font-bold bg-gray-100  active":"hover:text-green-900 hover:font-bold hover:bg-gray-50"}`}
                ...
                <ReusableInputDropdown  suggestions={questionCategories} onSelect={handleSelectedItemChange} />
            </div>
          ...
          <div>
            ...
            >
             { activeTab==="Objective"?"Add a new Objevtive Question":"Add a new Programming question"}
            </button>
          </div>
     <DataTable<QuestionObjective|QuestionProgramming >
      ...
      expandableRows
       ...
       highlightOnHover
       responsive
       paginationDefaultPage={activeTab==="Objective"?filterData.page.objective.page:filterData.page.programming.page}
